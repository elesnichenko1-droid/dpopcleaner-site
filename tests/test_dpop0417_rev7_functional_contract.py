...
class DPop0417Rev7FunctionalContractTests(unittest.TestCase):
    ...
    def test_frozen_zapret_update_controls_are_bridge_owned(self):
        proxy_path = ROOT / 'v0417/src/SimpleUpdate/ZapretUpdateProxyHost.cs'
        ...
        self.assertIn('LegacyCheckVersionButtonId', proxy)
        self.assertIn('LegacyDownloadButtonId', proxy)
        self.assertIn('CreateLegacyUpdateProxy', proxy)
        self.assertIn('Проверить версию', proxy)
        self.assertIn('Скачать и установить', proxy)
        self.assertIn('NativeBridge.ShowWindow(_legacyCheckVersionButton, NativeBridge.SW_HIDE)', proxy)
        self.assertIn('NativeBridge.ShowWindow(_legacyDownloadButton, NativeBridge.SW_HIDE)', proxy)
        self.assertIn('LegacyZapretUpdater.Run(_applicationRoot)', proxy)
        self.assertIn('RefreshDisplayedZapretVersion', proxy)
        self.assertIn('.service', proxy)
        self.assertIn('version.txt', proxy)
        self.assertIn('ZapretUpdateProxyHost', program)
        self.assertIn('zapretUpdateProxy.Show()', program)
