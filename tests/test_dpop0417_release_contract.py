...
class DPop0417ReleaseContractTests(unittest.TestCase):
    def test_site_manifest_and_publisher_are_one_0417_release(self):
        ...
        self.assertEqual(version['channel'], 'beta')

        beta = json.loads((ROOT / 'update/beta.json').read_text(encoding='utf-8'))
        self.assertEqual(beta['version'], '0.4.17')
        self.assertEqual(beta['version_code'], 417)
        self.assertEqual(beta['revision'], 1)
