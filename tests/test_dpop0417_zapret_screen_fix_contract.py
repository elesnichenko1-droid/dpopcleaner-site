...
class DPop0417ZapretScreenFixContractTests(unittest.TestCase):
    def test_rev4_preserves_zapret_screen_fix_as_a_staged_shell_companion(self):
        ...
        self.assertTrue(project.is_file(), 'ZapretScreenFix companion project is required in rev.4')
        self.assertTrue(patcher.is_file(), 'ZapretStrategyPatcher implementation is required in rev.4')
        self.assertTrue(command.is_file(), 'Zapret Screen Fix shell command is required in rev.4')
    ...
    def test_rev4_builds_and_tests_the_preserved_rev2_companion(self):
        ...
        self.assertIn('revision = 4', workflow)
        self.assertIn('v0.4.17-rev4', workflow)

    def test_rev4_release_notes_and_site_disclose_preserved_screen_share_fix(self):
