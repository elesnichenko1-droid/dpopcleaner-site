...
class DPop0417InstallerContractTests(unittest.TestCase):
    def test_installer_and_installed_smoke_are_exact_and_safe(self):
        ...
        self.assertIn('filename: "{app}\\simpleupdate.exe"; description: "запустить dpopcleaner"', lowered)
        self.assertIn('zapretscreenfix.exe', lowered)
        self.assertIn('filename: "{app}\\modules\\zapretscreenfix.exe"', lowered)
        ...
        self.assertIn('modules/zapretscreenfix.exe', smoke_text.replace('\\', '/'))
        self.assertIn('shell/commands/zapret-screen-fix.json', smoke_text.replace('\\', '/'))
        self.assertIn('zapret_screen_fix_present', smoke_text)
