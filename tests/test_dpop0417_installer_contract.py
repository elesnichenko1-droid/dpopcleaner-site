...
class DPop0417InstallerContractTests(unittest.TestCase):
    def test_installer_and_installed_smoke_are_exact_and_safe(self):
        ...
        self.assertIn('source: "{#stageroot}\\simpleupdate.exe"; destdir: "{app}"', lowered)
        self.assertIn('source: "{#stageroot}\\*.bat"; destdir: "{app}"', lowered)
        self.assertIn('source: "{#stageroot}\\bin\\*"; destdir: "{app}\\bin"', lowered)
        self.assertIn('source: "{#stageroot}\\lists\\*"; destdir: "{app}\\lists"', lowered)
        self.assertIn('source: "{#stageroot}\\utils\\*"; destdir: "{app}\\utils"', lowered)
        self.assertIn('source: "{#stageroot}\\.service\\*"; destdir: "{app}\\.service"', lowered)
        self.assertIn('source: "{#stageroot}\\license.txt"; destdir: "{app}"', lowered)
        self.assertIn('name: "{autoprograms}\\dpopcleaner"; filename: "{app}\\simpleupdate.exe"', lowered)
        self.assertIn('name: "{autodesktop}\\dpopcleaner"; filename: "{app}\\simpleupdate.exe"', lowered)
        ...
        self.assertIn("assert-file 'service.bat'", smoke_text)
        self.assertIn("assert-file 'general.bat'", smoke_text)
        self.assertIn("assert-file 'bin/winws.exe'", normalized)
        self.assertIn("assert-file 'bin/windivert64.sys'", normalized)
        self.assertIn("assert-file '.service/version.txt'", normalized)
