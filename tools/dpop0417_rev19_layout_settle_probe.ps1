...
function Get-RowTop($Buttons,[int[]]$Ids,[string]$Name){$row=@($Buttons|Where-Object{$Ids -contains $_.Id});if($row.Count -ne $Ids.Count){throw "$Name expected $($Ids.Count), found $($row.Count)."};$tops=@($row|Select-Object -ExpandProperty Top -Unique);if($tops.Count -ne 1){throw "$Name is not one row: $($tops -join ',')."};[int]$tops[0]}
...
try{
 ...
 $tab=Get-Children $window|Where-Object{$_.Visible -and $_.ClassName -eq 'Button' -and $_.Id -eq 905}|Select-Object -First 1
 ...
 Wait-Until 8 'rev.19 settle target buttons' {$c=@(Get-Children $window);@($c|Where-Object{$_.Visible -and $_.ClassName -eq 'Button' -and $targetIds -contains $_.Id}).Count -eq 19}
 foreach($size in @(@(1024,768),@(1366,800),@(1680,840),@(1908,950))){$w=[int]$size[0];$h=[int]$size[1];$y=if($h -gt $screenHeight){$screenHeight-$h-8}else{0};if(-not [Rev19SettleNative]::SetWindowPos($window,[IntPtr]::Zero,0,$y,$w,$h,0x0002 -bor 0x0004 -bor 0x0010 -bor 0x0400)){throw "settle resize ${w}x${h} failed"};Start-Sleep -Milliseconds 1100}
 for($i=0;$i -lt 16;$i++){
  $wb=[Rev19SettleNative]::Bounds($window);$children=@(Get-Children $window);$buttons=@($children|Where-Object{$_.Visible -and $_.ClassName -eq 'Button' -and $targetIds -contains $_.Id});if($buttons.Count -ne 19){throw "settle sample $i expected 19 buttons, found $($buttons.Count)"}
