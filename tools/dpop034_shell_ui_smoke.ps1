...
function Get-DirectChildren([IntPtr]$parent){
  $result=@(); $child=[DPop034ShellWin32]::GetWindow($parent,5)
  while($child -ne [IntPtr]::Zero){ $result += $child; $child=[DPop034ShellWin32]::GetWindow($child,2) }
  return $result
}
...
function Resize-Client([IntPtr]$h,[int]$w,[int]$height){
  $wr=New-Object DPop034ShellWin32+RECT; $cr=New-Object DPop034ShellWin32+RECT
  ...
  $ow=$w+(($wr.Right-$wr.Left)-($cr.Right-$cr.Left)); $oh=$height+(($wr.Bottom-$wr.Top)-($cr.Bottom-$cr.Top))
...
function Capture([IntPtr]$h,[string]$name){
  ...
  $bmp=New-Object Drawing.Bitmap($w,$height); $g=[Drawing.Graphics]::FromImage($bmp); $hdc=$g.GetHdc()
  try {
    if(-not [DPop034ShellWin32]::PrintWindow($h,$hdc,2)){ if(-not [DPop034ShellWin32]::PrintWindow($h,$hdc,0)){throw "PrintWindow failed: $name"} }
  } finally { $g.ReleaseHdc($hdc); $g.Dispose() }
  try { $path=Join-Path $OutputDir ($name+'.png'); $bmp.Save($path,[Drawing.Imaging.ImageFormat]::Png); return $path } finally { $bmp.Dispose() }
}
...
try {
  ...
  do { Start-Sleep -Milliseconds 250; $p.Refresh(); if($p.HasExited){throw 'App exited early'} } while($p.MainWindowHandle -eq 0 -and (Get-Date)-lt $deadline)
  ...
  $sections=[ordered]@{
    ...
    'Инструменты'='tools'
    'Zapret Center'='zapret'
    'Обновления'='updates'
    'Настройки'='settings'
  }
  foreach($label in $sections.Keys){ if((Find-DirectButton $main $label) -eq [IntPtr]::Zero){throw "Missing visible sidebar section: $label"} }

  $captures=[ordered]@{}
  foreach($label in $sections.Keys){
    Open-Section $main $label
    $key=$sections[$label]
    $captures[$key]=Capture $main ("sidebar-{0}-1100x700" -f $key)
  }

  [pscustomobject]@{
    target='DPopCleaner 0.3.4 BETA R2'
    visible_sidebar_sections=$sections.Count
    captures=$captures
  } | ConvertTo-Json -Depth 6 | Set-Content (Join-Path $OutputDir 'shell-ui-smoke-report.json') -Encoding utf8
