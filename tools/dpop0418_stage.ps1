...
if (-not (Test-Path -LiteralPath $allowlistPath -PathType Leaf)) { throw 'Missing v0418/stage-allowlist.txt.' }
...
foreach ($native in @($corePath, $updaterPath)) {
    if (-not (Test-Path -LiteralPath $native -PathType Leaf)) { throw "Required 0.4.18 native executable is missing: $native" }
}
...
if ($coreVersion -ne '0.4.18.2') { throw "Unexpected DPopCleaner.exe FileVersion: $coreVersion" }
if ($updaterVersion -ne '0.4.18.2') { throw "Unexpected DPopUpdater.exe FileVersion: $updaterVersion" }

$zapretRequiredFiles = @('LICENSE.txt','service.bat','general.bat','.service/version.txt','bin/winws.exe','bin/WinDivert.dll','bin/WinDivert64.sys')
if (-not (Test-Path -LiteralPath $preparedZapretPath -PathType Container)) { throw "Prepared pinned Zapret tree is missing: $preparedZapretPath" }
foreach ($relative in $zapretRequiredFiles) {
    ...
    if (-not (Test-Path -LiteralPath $candidate -PathType Leaf)) { throw "Prepared pinned Zapret tree missing required file: $relative" }
}
if (-not (Test-Path -LiteralPath (Join-Path $preparedZapretPath 'lists') -PathType Container)) { throw 'Prepared pinned Zapret tree missing required lists directory.' }
...
if (-not (Test-Path -LiteralPath $noticesPath -PathType Leaf)) { throw 'Missing v0418/third_party/THIRD_PARTY_NOTICES.txt.' }
...
foreach ($module in $moduleFiles) {
    ...
    elseif ($RequireCompanions) { throw "Required companion is missing: $($module.Dest)" }
}
...
foreach ($required in @('DPopCleaner.exe', 'DPopUpdater.exe', 'Documentation/THIRD_PARTY_NOTICES.txt')) {
    if (-not (Test-Path -LiteralPath (Join-Path $stageRoot $required) -PathType Leaf)) { throw "Staged file missing: $required" }
}
if ($RequireCompanions) {
    foreach ($module in $moduleFiles) {
        if (-not (Test-Path -LiteralPath (Join-Path $stageRoot $module.Dest) -PathType Leaf)) { throw "Staged companion missing: $($module.Dest)" }
    }
}
foreach ($relative in $zapretRequiredFiles) {
    $candidate = Join-Path (Join-Path $stageRoot 'ThirdParty/Zapret') ($relative -replace '/', [IO.Path]::DirectorySeparatorChar)
    if (-not (Test-Path -LiteralPath $candidate -PathType Leaf)) { throw "Staged bundled Zapret missing required file: $relative" }
}
if (-not (Test-Path -LiteralPath (Join-Path $stageRoot 'ThirdParty/Zapret/lists') -PathType Container)) { throw 'Staged bundled Zapret missing lists directory.' }
