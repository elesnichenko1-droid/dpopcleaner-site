...
namespace DPopCleaner.SimpleUpdate
{
    internal sealed class LauncherContext : ApplicationContext
    {
        ...
        {
            // The native DPopCleaner core already owns the Zapret controls and their
            // layout.  The three overlay hosts below create replacement child
            // windows and reposition the same controls; on some DPI/window sizes
            // those windows overlap and make the whole tab unusable.  Keep the
            // native page authoritative until the overlay implementation is fixed.
            if (_zapretHost != null) _zapretHost.Hide();
            if (_zapretVisualHost != null) _zapretVisualHost.Hide();
            if (_zapretResponsiveHost != null) _zapretResponsiveHost.Hide();
        }
